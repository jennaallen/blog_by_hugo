...
I created a function to perform the fuzzy text matching. One benefit of this function is that it allows the user to concatenate text to the football school names. This is important because several of the school names from the football data are shortened (e.g Arizona instead of The University of Arizona). The function allows users to concatenate text either before or after the football school name, if so desired. This can improve the fuzzy matching in some cases.
...
Users can filter the data based on various fields and then see the corresponding game stats for the filtered list of schools. The user can also select the years they want to view data for and can choose to view the means or medians of the data for the given years selected. 

The Tableau dashboard can be found [here.](https://public.tableau.com/views/football_school_dataviz/SchoolSelectionDashboard?:embed=y&:display_count=yes)
